...
export const getAllSubnotes = createAsyncThunk<Note[], void, { rejectValue: Error }>(
    'subnotes/getAllSubnotes',
    async (_, { rejectWithValue }) => {
        try {
            const token = localStorage.getItem('token') as string;
            const decodedToken = jwtDecode(token) as Record<string, unknown>;
            const email = decodedToken.email as string;
            const response = await fetch('http://localhost:3000/subnotes/get/' + email, {
                headers: {
